...
train_transform = transforms.Compose([
    ...
    transforms.RandomRotation(30),          # tilt digits
    transforms.RandomAffine(
        degrees=0,
        translate=(0.1, 0.1),               # shift around
...
loaders = {
    "train" : DataLoader(train_data, 100, True, num_workers=1),
    "test" : DataLoader(test_data, 100, True, num_workers=1),
}
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(loaders["train"]):
        ...
        optimizer.step()
        if batch_idx % 50 == 0:
            print(f"Train Epoch: {epoch} \n {batch_idx*len(data)}/{len(loaders['train'].dataset)} ({100. * batch_idx / len(loaders['train']):.0f}%)\t{loss.item():.6f}")
...
if __name__ == "__main__":
    for epoch in range(1, int(input("Epochs: "))+1):
        train(epoch)
    test()
